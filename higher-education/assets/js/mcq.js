/* =========================================================================
   CBT Engine - DSSSB TGT CS Competitive Exam Engine
   Features: Dedicated Domain Live Indicator, 4-Slot Carousel Viewport
   with Auto-Center Scrolling, and Marking (+1.0 / -0.25).
   ========================================================================= */
const $ = id => document.getElementById(id);

const isProctoringEnabled    = () => window.CBT_CONFIG?.ENABLE_PROCTORING ?? false;
const getCorrectMarks        = () => Number(window.CBT_CONFIG?.MARKS_CORRECT ?? 1.0);
const getIncorrectMarks      = () => Number(window.CBT_CONFIG?.MARKS_INCORRECT ?? 0.25);
const getPenaltyMarks        = () => Number(window.CBT_CONFIG?.PENALTY_WARNING ?? 1.0);
const getTestName            = () => window.CBT_CONFIG?.TEST_NAME ?? "Computer Organisation and Architecture";
const getDomainName          = () => window.CBT_CONFIG?.DOMAIN_NAME ?? "www.SinghClasses.in";
const getSubjectDomain       = () => window.CBT_CONFIG?.SUBJECT_DOMAIN ?? "";
...
window.CBTState = {
  ...
  currentYearIndex: 0,
  currentQuestion: 0,
  studentNameVal: "",     // Candidate Full Name
  stateVal: "Delhi",      // Candidate State / UT
  categoryVal: "UR",      // Reservation Category
  studentName: "",        // Formatted combined string
  userAnswers: [],
  visitedQuestions: [],
  lockedAnswers: [],
  sectionTimes: [],
...
function getSingleSessionKey() {
  return `cbt_dsssb_${getTestName().replace(/\s+/g, '_')}`;
}
...
function saveSessionToLocalStorage() { 
  ...
  try { 
    const payload = { 
      ...
      stateVal: CBTState.stateVal, 
      categoryVal: CBTState.categoryVal, 
...
function restoreSession(data) {
  Object.assign(CBTState, {
    ...
    stateVal: data.stateVal || "Delhi",
    categoryVal: data.categoryVal || "UR",
  ...
  if ($('candidate-name-input')) $('candidate-name-input').value = CBTState.studentNameVal;
  if ($('candidate-state-input')) $('candidate-state-input').value = CBTState.stateVal;
  if ($('candidate-category-input')) $('candidate-category-input').value = CBTState.categoryVal;
...
function escapeHTML(str) { return str == null ? "" : str.toString().replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/\n/g, "<br>"); }
    ...
    try {
      ...
      if (raw[0] && Array.isArray(raw[0].questions)) {
        CBTState.listExamPapers = raw.map(paper => ({
          title: getVerbatim(paper, ['title', 'Title', 'sectiontitle', 'SectionTitle'], "Discipline Section"),
          year: getVerbatim(paper, ['year', 'Year', 'section', 'Section', 'part', 'Part'], "Part A"),
          questions: (paper.questions || []).map(q => {
            ...
            return {
              ...
              tag: getVerbatim(q, ['tag', 'Tag', 'info', 'Info', 'topic', 'Topic'], "").toString(),
              options: cleanOpts,
        ...
        raw.forEach(row => {
          ...
          let sectionLabel = getVerbatim(r, ['section', 'year', 'part', 'set'], "Discipline 1").toString().trim();
          ...
          let qTag = getVerbatim(r, ['tag', 'info', 'topic'], "").toString();
...
function triggerVerifyModal(type) { 
  ...
  if (type === 'pdf') { 
    ...
    if (heading) heading.innerText = "Open Topic Revision Notes?"; 
    if (text) text.innerText = "Open lecture notes inside a new browser tab."; 
    if (actionBtn) {
    ...
    if (heading) heading.innerText = "Watch Lecture Video?"; 
    if (text) text.innerText = "Launch the DSSSB TGT revision video on YouTube."; 
    if (actionBtn) {
...
/* Step Navigation for DSSSB Registration */
window.goToGuidelinesStep = () => { 
  CBTState.studentNameVal = ($('candidate-name-input') ? $('candidate-name-input').value.trim() : "").toUpperCase() || "CANDIDATE"; 
  CBTState.stateVal = $('candidate-state-input') ? $('candidate-state-input').value : "Delhi"; 
  CBTState.categoryVal = $('candidate-category-input') ? $('candidate-category-input').value : "UR"; 
  CBTState.studentName = `${CBTState.studentNameVal} | ${CBTState.stateVal} | ${CBTState.categoryVal}`; 
...
window.onload = async () => { 
  ...
  const domainWatermark = $('domain-watermark-text');
  if (domainWatermark) {
    const subDom = getSubjectDomain();
    domainWatermark.innerText = subDom ? `${getDomainName()} / ${subDom}` : getDomainName();
  }

  const correctLbl = $('welcome-correct-lbl');
  if (correctLbl) correctLbl.innerText = `+${getCorrectMarks().toFixed(2)} Correct`; 
  
  const incorrectLbl = $('welcome-incorrect-lbl');
  if (incorrectLbl) incorrectLbl.innerText = `-${getIncorrectMarks().toFixed(2)} Wrong`; 

  const savedData = getSavedSession();
  if (savedData) {
    CBTState.pendingRestoreData = savedData;
    const subtext = $('resume-modal-subtext');
    if (subtext) subtext.innerText = `A previous attempt for "${savedData.studentNameVal}" was found. Would you like to resume?`;
    if ($('modal-resume')) $('modal-resume').style.display = 'flex';
  } else {
    if ($('modal-welcome')) $('modal-welcome').style.display = 'flex';
  }
  
  const nameInput = $('candidate-name-input');
  if (nameInput) {
    nameInput.addEventListener('focus', () => loadQuestionsFromSheet());
    nameInput.addEventListener('keypress', e => { if (e.key === 'Enter') goToGuidelinesStep(); }); 
  }
...
window.beginExam = async () => { 
  const rawNameInput = $('candidate-name-input') ? $('candidate-name-input').value.trim() : ""; 
  CBTState.studentNameVal = rawNameInput.toUpperCase() || "CANDIDATE"; 
  CBTState.stateVal = $('candidate-state-input') ? $('candidate-state-input').value : "Delhi"; 
  CBTState.categoryVal = $('candidate-category-input') ? $('candidate-category-input').value : "UR"; 
  CBTState.studentName = `${CBTState.studentNameVal} | ${CBTState.stateVal} | ${CBTState.categoryVal}`; 
  ...
  if (CBTState.listExamPapers.length === 0) { 
    ...
    if (CBTState.listExamPapers.length === 0) {
      ...
      if ($('modal-welcome')) $('modal-welcome').style.display = 'flex';
      alert("Network request timed out. Please check your connection and restart.");
      return;
    }
  } 
...
window.addEventListener("beforeunload", () => { 
  if (CBTState.isExamActive && CBTState.sections[CBTState.currentYearIndex] && !CBTState.sections[CBTState.currentYearIndex].submitted) { 
    ...
    p.append("candidateName", (CBTState.studentNameVal || "CANDIDATE") + " (Dropout)"); 
    p.append("state", CBTState.stateVal); 
    p.append("category", CBTState.categoryVal); 
    p.append("testName", getTestName()); 
    p.append("currentSection", s.year + " - " + s.title); 
    p.append("obtainedScore", sc.toFixed(2)); 
...
function applySecurityPenalty() { 
  ...
  if ($('warning-count-display')) $('warning-count-display').innerText = `Total Warnings: ${CBTState.securityWarnings} (Penalty: -${(CBTState.securityWarnings * getPenaltyMarks()).toFixed(2)} Marks)`; 
...
/* Build Navigation Bar with 4-Tab Window and Exact Step Auto-Centering */
window.buildYearNav = () => { 
  ...
  CBTState.sections.forEach((p, idx) => { 
    ...
    t.innerHTML = `<span class="tab-letter-pill">${escapeHTML(p.year)}</span><span class="tab-title-text">${escapeHTML(p.title)}</span>`; 
    ...
    t.onclick = async () => { 
      if (CBTState.sections[idx].submitted || idx === CBTState.currentYearIndex) { 
        ...
        showToastAlert("Submit the active section before moving forward."); 
      } 
    }; 
    c.appendChild(t); 
  }); 

  /* Auto-scroll track: exactly 4 tabs visible, sliding smoothly as sections advance */
  requestAnimationFrame(() => {
    const activeTab = c.querySelector('.year-tab.active');
    if (activeTab) {
      const activeIdx = CBTState.currentYearIndex;
      const tabWidth = c.clientWidth / 4;
      const targetScroll = Math.max(0, (activeIdx - 1) * tabWidth);
...
function renderSidebarToppers(toppersArray) { 
  ...
  let realToppers = toppersArray.filter(t => t && (t.candidateName || t.studentName || t.name) && (t.studentName !== "Awaiting..." && t.name !== "Awaiting..."));
  ...
  listEl.innerHTML = top7.map((t, idx) => {
    ...
    const name = escapeHTML(t.candidateName || t.studentName || t.name || 'Candidate');
    const marks = getMarks(t).toFixed(2);
    const stateVal = t.state || '';
    const stateBadge = stateVal && stateVal !== '-' ? ` · [${escapeHTML(stateVal)}]` : '';
    return `<span class="topper-badge">${medal} <strong>${name}</strong> · ${marks} Marks${stateBadge}</span>`;
  }).join(' '); 
...
window.loadQuestion = () => { 
  ...
  if (CBTState.questions[CBTState.currentQuestion].image) baseQuestionText += `<div class="question-image-wrap" style="margin:0 0 12px 0;text-align:left;max-width:100%;display:flex;justify-content:flex-start;align-items:center;"><img src="${CBTState.questions[CBTState.currentQuestion].image}" alt="Question Graphic" style="max-width:100%;max-height:220px;border-radius:8px;border:1px solid var(--border-color);object-fit:contain;display:block;"></div>`; 
...
function animateCount(el, target) { 
  ...
  function step(timestamp) { 
    ...
    el.innerText = (val % 1 === 0) ? Math.round(val) : val.toFixed(2); 
    if (progress < duration) requestAnimationFrame(step); 
    else el.innerText = (target % 1 === 0) ? target : target.toFixed(2); 
  } 
...
window.showSubmitModal = () => { 
  ...
  if (u > 0) showToastAlert(`${u} Question(s) still unattempted.`); 
  if ($('submit-modal-text')) $('submit-modal-text').innerText = `Are you sure you want to lock and submit ${CBTState.sections[CBTState.currentYearIndex].year}?`; 
  CBTState.isTimerPaused = true; 
...
window.processSectionSubmission = async function() { 
  ...
  if ($('lbl-user-greeting')) $('lbl-user-greeting').innerText = CBTState.studentNameVal || "CANDIDATE"; 
  if ($('lbl-section-title')) $('lbl-section-title').innerText = `${sec.year} Evaluated,`; 
  ...
  if ($('lbl-keep-going-msg')) $('lbl-keep-going-msg').innerText = nextSecName ? "Keep going." : "All parts submitted!"; 
  ...
  if ($('lbl-score-obtained')) $('lbl-score-obtained').innerText = secMarks.toFixed(2); 
  if ($('lbl-score-total')) $('lbl-score-total').innerText = totalSectionMarks.toFixed(0); 
  ...
  if (tg) { 
    ...
    CBTState.sections.forEach((s, sIdx) => { 
      ...
      let pR = sM > 0 && sS > 0 ? ((sS / sM) * 100).toFixed(1) : "0.0"; 
      if (!s.submitted) pR = "0.0"; 
      ...
      tg.innerHTML += `
        <div class="summary-row-card row-theme-${themeClass}">
          <div class="sm-col sm-col-section">
            <div class="sec-card-icon-box ${themeClass}">⚡</div>
          ...
          <div class="sm-col">
            ...
            <span class="sm-subtext">(${s.submitted ? sS.toFixed(2) : 0} / ${sM})</span>
          </div>
          <div class="sm-col">
    ...
    let fP = cM > 0 ? ((aS / cM) * 100).toFixed(1) : "0.0"; 
    ...
    tg.innerHTML += `
      <div class="summary-row-card row-theme-purple total-row">
        <div class="sm-col sm-col-section">
          <div class="sec-card-icon-box purple">Σ</div>
          <div class="sec-card-title-group">
        ...
        <div class="sm-col">
          ...
          <span class="sm-subtext">(${aS.toFixed(2)} / ${cM})</span>
        </div>
        <div class="sm-col">
  ...
  CBTState.globalFormPayload = { 
    ...
    candidateName: CBTState.studentNameVal || "CANDIDATE", 
    state: CBTState.stateVal, 
    category: CBTState.categoryVal, 
    testName: getTestName(), 
    currentSection: `${sec.year} - ${sec.title}`, 
    obtainedScore: secMarks.toFixed(2), 
  ...
  if (saveUrl && CBTState.globalFormPayload) { 
    ...
    try { 
      ...
      showToastAlert("Score saved to database successfully!"); 
    } catch (err) { 
      console.warn("Apps Script Save Note:", err); 
    } 
  } 
  ...
  if (b) { 
    ...
    b.style.opacity = '1'; 
    if (CBTState.currentYearIndex < CBTState.sections.length - 1) { 
      let nextLabel = CBTState.sections[CBTState.currentYearIndex + 1].year || `PART ${CBTState.currentYearIndex + 2}`; 
      b.innerHTML = `CONTINUE TO ${nextLabel.toUpperCase()} →`; 
    } else { 
      b.innerHTML = `VIEW COMPLETE EVALUATION`; 
    } 
  } 
};
