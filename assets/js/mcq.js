...
window.addEventListener("beforeunload", e => {
    if (isExamActive && sections[currentYearIndex] && !sections[currentYearIndex].submitted) {
...
function renderSidebarToppers(toppersArray) {
    ...
    // Extract total marks / obtained score from various possible API field names
    const getMarks = (t) => {
        let val = t.obtainedScore ?? t.score ?? t.totalMarks ?? t.marks ?? 0;
        return parseFloat(val) || 0;
    };

    // Extract accuracy percentage for tie-breaking
    const getAccuracy = (t) => {
    ...
    // Take top 7 performers
    const top7 = realToppers.slice(0, 7);

    listEl.innerHTML = top7.map((t, idx) => {
        const rank = idx + 1;
        const medal = rank === 1 ? '🥇' : (rank === 2 ? '🥈' : (rank === 3 ? '🥉' : `#${rank}`));
        const name = escapeHTML(t.name || t.studentName || 'Student');
...
function updateTimerDisplay() {
    ...
    if (sections[currentYearIndex] && sections[currentYearIndex].submitted) {
    ...
    [bDesktop, bMobile].forEach(b => { if (b) { b.className = targetClass; b.style.color = (sections[currentYearIndex] && sections[currentYearIndex].submitted) ? '' : targetStyleColor; } });
}

function startTimer() {
    if (timerInterval) clearInterval(timerInterval);
    timerInterval = setInterval(() => {
        if (isTimerPaused || !isExamActive || (sections[currentYearIndex] && sections[currentYearIndex].submitted)) return;
